...
def read_examples(filename):
    ...
    examples=[]
    source_folder, target_folder = filename.split(',')
    lang = 'python'

    source_contents = []
    source_file_list = glob.glob(os.path.join(source_folder, '*.py'))

    for file_path in source_file_list:
        with open(file_path, 'r') as file:
            source_contents.append(file.read())

    target_contents = []
    target_file_list = glob.glob(os.path.join(target_folder, '*.txt'))

    for file_path in target_file_list:
        with open(file_path, 'r') as file:
            target_contents.append(file.read())

    for i in range(len(source_file_list)):
        inp = source_contents[i].strip()
        oup = target_contents[i].strip()
        examples.append(Example(source = inp, target = oup, lang = lang))
...
def main():
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [ int(args.num_train_epochs*(i+1)//20) for i in range(20)] and ((epoch + 1) % 4) == 0:
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        for pred in preds:
                            ...
                            p.append(text)
                model.train()
                if not os.path.exists(os.path.join(args.output_dir,"dev.output")):
                    os.mkdir(os.path.join(args.output_dir,"dev.output"))  #path is where you want dev_output directory to be created
                if not os.path.exists(os.path.join(args.output_dir,"dev.gold")):
                    os.mkdir(os.path.join(args.output_dir,"dev.gold"))    #path is where you want dev_gold directory to be created
                i=0
                #with open(os.path.join(args.output_dir,"dev.output"),'w') as f, open(os.path.join(args.output_dir,"dev.gold"),'w') as f1: #lite 
                predictions = []
                accs = []
                for ref,gold in zip(p,eval_examples):
                    predictions.append(ref)
                    with open(os.path.join(args.output_dir, "dev.output", str(i) + '.txt'),'w') as writefile:
                        writefile.write(ref)
                    with open(os.path.join(args.output_dir, "dev.gold", str(i) + '.txt'),'w') as writefile:
                        writefile.write(gold.target)
                    i=i+1
                    #f.write(ref+'\n') #lite
    ...
    if args.do_test:
        ...
        for idx,file in enumerate(files):   
            ...
            if not os.path.exists(os.path.join(args.output_dir,"test_{}.output".format(str(idx)))):
                os.mkdir(os.path.join(args.output_dir,"test_{}.output".format(str(idx))))
            if not os.path.exists(os.path.join(args.output_dir,"test_{}.gold".format(str(idx)))):
                os.mkdir(os.path.join(args.output_dir,"test_{}.gold".format(str(idx))))

            i=0
            for ref,gold in zip(p,eval_examples):
                predictions.append(ref)
                with open(os.path.join(args.output_dir, 'test_{}.output'.format(str(idx)), str(i) + '.txt'),'w') as writefile:
                    writefile.write(ref)
                with open(os.path.join(args.output_dir, 'test_{}.gold'.format(str(idx)), str(i) + '.txt'),'w') as writefile:
                    writefile.write(gold.target)
                i=i+1
                accs.append(ref==gold.target)

            # open(os.path.join(args.output_dir, 'test_{}.output'.format(str(idx)), str(i) + '.txt'),'w')
            # open('..path'+'/'+'test_{}.output'.format(str(idx))+'/'+str(i)+'.txt', 'w')
